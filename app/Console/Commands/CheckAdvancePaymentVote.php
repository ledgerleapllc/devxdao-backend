...
class CheckAdvancePaymentVote extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'advance-payment-vote:check';
     ...
     * @var string
     */
    protected $description = 'Check Advance Payment Vote';
    ...
    public function checkInformal($settings, $mins, $minMembers, $vote, $force = false) {
        ...
        if ($start->lt($today) || $force) {
            ...
            if ($vote->result_count < $minMembers) {
                ...
                if ($result == "success") {
                    if ($proposal->type == 'advance-payment'
                        && ($settings['autostart_advance_payment_formal_votes'] ?? null) == 'yes') {
    ...
    public function checkFormal($settings, $mins, $vote, $force = false) {
        ...
        $voteInfomal = Vote::where('proposal_id', $vote->proposal_id)->where('type', 'informal')
            ->where('content_type', 'advance-payment')->first();
    ...
    public function handle()
    {
        ...
        $informals = Vote::has('proposal')
                        ->where('content_type', 'advance-payment')
                        ->where('type', 'informal')
                        ->where('status', 'active')
